        ...
        <div class="table-box">
            <el-table :data="tableData" v-loading="loading" element-loading-text="拼命加载中" fit
                ...
                <el-table-column prop="degree" label="重要性" width="75" align="center"
                    ...
                    <template slot-scope="scope">
                        <el-popover
                            ...
                            <div slot="reference">
                                <span v-if="scope.row.degree===4" class="text-danger">核心</span>
                                <span v-if="scope.row.degree===3" class="text-important">重要</span>
                                <span v-if="scope.row.degree===2" class="co-666">常规</span>
                                <span v-if="scope.row.degree===1" class="co-999">次要</span>
                            </div>
                        </el-popover>
                    </template>
                </el-table-column>
                <el-table-column prop="type_text" label="客户类型" width="95">
                    <template slot-scope="scope">
                        <span :class="scope.row.type===1?'text-link':''">{{scope.row.type_text}}</span>
                    </template>
                </el-table-column>
                ...
                <el-table-column prop="industry_text" label="所属行业" width="120" :show-overflow-tooltip="true">
                </el-table-column>
...
<script>
    // import { page, caseShare } from 'components'
    ...
    export default {
        ...
        methods: {
            ...
            formatSearchData(code,routeName){
                ...
                let query = {}
                query.follower = routeName === 'clientMy' ? store.state.user_info.user.id : ''
                query.isTeam = routeName === 'clientMy' ? 0 : 1
                if (code === 'undefined'){
                    //在我的/团队中（全部-0，企业-1，个人-2），默认显示0
                    query.mark = 0
                }else {
            ...
            getDefaultList () {
                ...
                this.params = {
                    sdata: JSON.stringify(searchData),
                }
                this.getList()
            },
            //分页操作
            ...
            handelDel (index, id) {
                this.$fetch.api_client.clientDel(id)
                    .then(({
                               msg,
                               code
                           }) => {
